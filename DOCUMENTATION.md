...
First, downloaded the csv files from the EUR-lex [website](https://eur-lex.europa.eu/homepage.html), then helped Mohit Shreshtha with the fetching and pre-processing part. Second, set up of the project's structure. Third, created a module for different splitters implemented in [LlamaIndex](https://www.llamaindex.ai/). Fourth, merged different modules created by different members into a single evaluation pipeline. Fifth, created the command prompt client for normal user use mentioned in [README](https://github.com/mmshress/INLP-WS23/blob/project_report/README.md). Sixth, completed evaluations, collected metrics with Siddhant Tripathi. Seventh, created different plots that should be used for analysis, to decide a configuration to be used for final application.
